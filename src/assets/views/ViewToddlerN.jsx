...
import PasswordPopupN from '../../components/PasswordPopupN'; // Importar el componente de la contraseña
import {
  Button,
  CircularProgress,
  Typography,
...
export default function ViewToddlerN() {


  ///POP UPPOP UPPOP UPPOP UPPOP UPPOP UP
  const [isUnlockedN, setIsUnlockedN] = useState(false);
  const [timer, setTimer] = useState(null);

  // Recuperar el estado de localStorage al iniciar
  useEffect(() => {
    const storedIsUnlocked = localStorage.getItem('isUnlockedN');
    if (storedIsUnlocked === 'true') {
      ...
      if (expirationTime && Date.now() > expirationTime) {
        setIsUnlockedN(false); // Bloquear la pantalla si ha pasado el tiempo
        localStorage.removeItem('isUnlockedN');
  ...
  const unlockScreen = () => {
    ...
    localStorage.setItem('isUnlockedN', 'true');
  ...
  const gradeLinks = [
    ['ToddlerN', 'https://docs.google.com/spreadsheets/d/1GnJHgU2PwNH_WC-_eSErKQrI0LEbi3db/edit?usp=sharing&ouid=109106641591153354661&rtpof=true&sd=true'], // Enlace vacío para Toddler
    ['2KN', 'https://docs.google.com/spreadsheets/d/1ptk9PBnfdoujQF0f0-eeolQ9MiiHSFgdk0DrT5aKj30/edit?usp=sharing'],
    ['3KN', 'https://docs.google.com/spreadsheets/d/1nC_cK4nVeQOILF1P7lH-9oBQqdH4s58I6cnd5KWwlrw/edit?usp=sharing'],
    ['4KN', 'https://docs.google.com/spreadsheets/d/14JpubcbDAjAkc4H8Wa6vWaN7SZ2cCTbKwG17cjyxZio/edit?usp=sharing'],
    ['DAN', 'https://docs.google.com/spreadsheets/d/1k8O7Y8DTItYE5UCUdMgHRRWPJhcMYNyn6cccl_e80Nk/edit?usp=sharing'], // Enlace vacío para Toddler
  ];
  return (
    <div>
    {!isUnlockedN ? (
      <PasswordPopupN onPasswordSubmit={unlockScreen} /> // Mostrar el popup de contraseña si no está desbloqueado
    ) : (
    <Box
      sx={{
        display: 'flex',
  ...
  <Box key={grade} sx={{ width: '100%', mb: 2 }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
      ...
      >
        ...
        >
          {isLoading ? 'Procesando...' : `Upload Excel File | ${grade === 'DAN' ? 'Developmental Assessment' : grade.slice(0, -1)}`}
        </Button>
      </label>
